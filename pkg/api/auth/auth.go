...
func (a Auth) Authenticate(c echo.Context, user, pass string) (secundusapi.AuthToken, error) {
	u, err := a.udb.FindByUsername(a.db, user)
	if err != nil {
		return secundusapi.AuthToken{}, err
	}

	if !a.sec.HashMatchesPassword(u.Password, pass) {
		return secundusapi.AuthToken{}, ErrInvalidCredentials
	}

	if !u.Active {
		return secundusapi.AuthToken{}, secundusapi.ErrUnauthorized
	}
	...
	return secundusapi.AuthToken{Token: token, RefreshToken: u.Token}, nil
}
...
func (a Auth) Me(c echo.Context) (secundusapi.User, error) {
	au := a.rbac.User(c)
