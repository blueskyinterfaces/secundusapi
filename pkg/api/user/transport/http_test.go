...
import (
	...
	"github.com/secundusteam/secundus/pkg/api/user/transport"
...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID secundus.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr secundus.User) (secundus.User, error) {
					usr.ID = 1
...
func TestList(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *secundus.ListQuery, p secundus.Pagination) ([]secundus.User, error) {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (secundus.User, error) {
					return secundus.User{
						Role: &secundus.Role{
							AccessLevel: secundus.CompanyAdminRole,
						},
					}, nil
				},
			},
