...
import (
	...
	"github.com/blueskyinterfaces/secundusapi/pkg/api/user/transport"
...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID secundusapi.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr secundusapi.User) (secundusapi.User, error) {
					usr.ID = 1
...
func TestList(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *secundusapi.ListQuery, p secundusapi.Pagination) ([]secundusapi.User, error) {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (secundusapi.User, error) {
					return secundusapi.User{
						Role: &secundusapi.Role{
							AccessLevel: secundusapi.CompanyAdminRole,
						},
					}, nil
				},
			},
