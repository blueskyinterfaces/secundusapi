...
func (u User) Create(c echo.Context, req secundusapi.User) (secundusapi.User, error) {
	if err := u.rbac.AccountCreate(c, req.RoleID, req.CompanyID, req.LocationID); err != nil {
		return secundusapi.User{}, err
	}
...
func (u User) List(c echo.Context, p secundusapi.Pagination) ([]secundusapi.User, error) {
	au := u.rbac.User(c)
	q, err := query.List(au)
	if err != nil {
		return nil, err
...
func (u User) View(c echo.Context, id int) (secundusapi.User, error) {
...
func (u User) Update(c echo.Context, r Update) (secundusapi.User, error) {
	if err := u.rbac.EnforceUser(c, r.ID); err != nil {
		return secundusapi.User{}, err
	}

	if err := u.udb.Update(u.db, secundusapi.User{
		Base:      secundusapi.Base{ID: r.ID},
