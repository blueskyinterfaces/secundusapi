...
func (u User) Create(c echo.Context, req secundus.User) (secundus.User, error) {
	if err := u.rbac.AccountCreate(c, req.RoleID, req.CompanyID, req.LocationID); err != nil {
		return secundus.User{}, err
	}
...
func (u User) List(c echo.Context, p secundus.Pagination) ([]secundus.User, error) {
	au := u.rbac.User(c)
	q, err := query.List(au)
	if err != nil {
		return nil, err
...
func (u User) View(c echo.Context, id int) (secundus.User, error) {
...
func (u User) Update(c echo.Context, r Update) (secundus.User, error) {
	if err := u.rbac.EnforceUser(c, r.ID); err != nil {
		return secundus.User{}, err
	}

	if err := u.udb.Update(u.db, secundus.User{
		Base:      secundus.Base{ID: r.ID},
